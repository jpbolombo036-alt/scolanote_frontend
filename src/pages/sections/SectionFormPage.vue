<template>
  <div class="space-y-6 font-['Plus_Jakarta_Sans',sans-serif]">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ isEdit ? 'Modifier' : 'Créer' }} une section</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Accueil / Sections / {{ isEdit ? 'Modifier' : 'Nouvelle section' }}</p>
      </div>
...
onMounted(async () => {
  try {
    if (isEdit) {
      const res = await api.get(`/api/sections/${route.params.id}`)
...
async function onSubmit() {
  ...
  try {
    const payload = { ...form }
    if (isEdit) {
      await api.put(`/api/sections/${route.params.id}`, payload)
    } else {
