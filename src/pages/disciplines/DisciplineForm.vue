  ...
  <div v-if="visible" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">{{ discipline ? 'Modifier' : 'Créer' }} une discipline</h2>
      ...
      <form @submit.prevent="onSubmit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Conduite</label>
            <input v-model="form.conduite" type="text" class="w-full border rounded-lg px-3 py-2" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Application</label>
            <input v-model="form.application" type="text" class="w-full border rounded-lg px-3 py-2" />
          </div>
          <div class="md:col-span-2">
            <label class="block text-sm font-medium text-gray-700 mb-1">Observation</label>
            <textarea v-model="form.observation" rows="3" class="w-full border rounded-lg px-3 py-2"></textarea>
          </div>
        </div>
