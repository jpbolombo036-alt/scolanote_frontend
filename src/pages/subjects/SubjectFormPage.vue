<template>
  <div class="space-y-6 font-['Plus_Jakarta_Sans',sans-serif]">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ isEdit ? 'Modifier' : 'Créer' }} une matière</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Accueil / Matières / {{ isEdit ? 'Modifier' : 'Nouvelle matière' }}</p>
      </div>
...
onMounted(async () => {
  if (!isEdit) return
  try {
    const response = await api.get(`/api/matieres/${route.params.id}`)
...
async function onSubmit() {
  ...
  try {
    const payload = { ...form }
    if (isEdit) {
      await api.put(`/api/matieres/${route.params.id}`, payload)
    } else {
      await api.post('/api/matieres', payload)
    }
