  ...
  <div v-if="visible" class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all">
    <div class="bg-white dark:bg-[#0d1527] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800/80">
        <h2 class="text-lg font-extrabold text-slate-900 dark:text-white">{{ curriculum ? 'Modifier' : 'Créer' }} un programme</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div class="relative">
                ...
                <select v-model="form.levelId" class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 text-slate-900 dark:text-white rounded-xl pl-10 pr-10 py-2.5 text-xs font-medium outline-none focus:border-emerald-500 transition appearance-none">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <select v-model="form.sectionId" class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 text-slate-900 dark:text-white rounded-xl pl-10 pr-10 py-2.5 text-xs font-medium outline-none focus:border-emerald-500 transition appearance-none">
                  <option value="">Sélectionner</option>
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                placeholder="Ex: Programme Scientifique"
...
import { X, BookOpen, School, List, AlertCircle, Check } from 'lucide-vue-next'
...
watch(() => props.curriculum, (newCurriculum) => {
  if (newCurriculum) {
    Object.assign(form, newCurriculum)
  } else {
    Object.assign(form, { levelId: null, sectionId: null, optionId: null, nom: '' })
  }
}, { immediate: true })
