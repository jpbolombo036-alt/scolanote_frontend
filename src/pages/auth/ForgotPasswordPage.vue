      ...
      <div class="flex-1 flex flex-col justify-center px-6 sm:px-10 lg:px-16 xl:px-24 py-10">
        <div class="w-full max-w-md mx-auto">
          <div class="mb-8">
            ...
            <h2 class="text-3xl font-extrabold text-ink">Mot de passe oublié</h2>
            <p class="text-ink-soft text-sm mt-1">Entrez votre email pour recevoir un lien de réinitialisation</p>
          </div>
          <form @submit.prevent="forgotPassword" class="space-y-5">
            <div>
              <label class="block text-sm font-semibold text-ink mb-1.5">Adresse email</label>
              <div class="relative">
                <Mail class="w-5 h-5 text-ink-muted absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                <input
                  v-model="form.email"
                  type="email"
                  required
...
async function forgotPassword() {
  ...
  success.value = null
  try {
    await apiForgotPassword(form.value.email)
