<template>
  <div class="space-y-6 font-['Plus_Jakarta_Sans',sans-serif]">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ isEdit ? 'Modifier' : 'Créer' }} une affectation</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Accueil / Affectations / {{ isEdit ? 'Modifier' : 'Nouvelle affectation' }}</p>
      </div>
      ...
      <form @submit.prevent="onSubmit" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select v-model="form.teacherId" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 text-xs font-medium outline-none focus:border-emerald-500 transition appearance-none">
...
onMounted(async () => {
  try {
    ...
    if (assignmentRes) {
      Object.assign(form, assignmentRes)
    }
  } catch (e) {
...
async function onSubmit() {
  ...
  try {
    if (isEdit) {
      await api.put(`/api/attributions-enseignement/${route.params.id}`, form)
    } else {
      await api.post('/api/attributions-enseignement', form)
    }
