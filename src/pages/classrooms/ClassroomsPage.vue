<template>
  <div class="p-6">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="classroom in classrooms" :key="classroom.id">
            <td class="px-6 py-4 whitespace-nowrap font-medium">{{ classroom.nom }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ classroom.academicYearId }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ classroom.levelId }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-right">
              <button @click="openEditForm(classroom)" class="text-primary-600 hover:text-primary-800 mr-2">Modifier</button>
              <button @click="deleteClassroom(classroom.id)" class="text-red-600 hover:text-red-800">Supprimer</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <ClassroomForm v-if="showForm" :classroom="editingClassroom" @save="saveClassroom" @close="showForm = false" />
  </div>
</template>
...
async function saveClassroom(data) {
  try {
    if (editingClassroom.value) {
      await axios.put(`/api/salles/${editingClassroom.value.id}`, data)
    } else {
